          ...
          {rooms.map(room => (
            <div key={room._id} className="p-6 flex justify-between items-center group" style={{ backgroundColor: COLORS.bgSurface, border: `1px solid ${COLORS.border}` }}>
              ...
              <div className="flex items-center space-x-8">
                ...
                {/* 🛡️ 核心权限逻辑 */}
                {isAdmin ? (
                  <button className="text-[10px] uppercase tracking-widest text-red-500 hover:text-red-400 font-bold" onClick={() => alert('Permanently Deleting...')}>
                    Delete
                  </button>
                ) : (
                  <button 
                    className="text-[10px] uppercase tracking-widest text-amber-500 border border-amber-500/30 px-3 py-1 hover:bg-amber-500/10" 
                    onClick={() => alert('Request to delete "' + room.name + '" has been sent to Admin for approval. 👑')}
                  >
                    Request Deletion
                  </button>
                )}
