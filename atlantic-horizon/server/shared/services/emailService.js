...
class EmailService {
  ...
  async sendBookingEmail(recipientEmail, booking) {
    ...
    const defaultTemplate = `Dear {{guest_name}}, Your stay at The Atlantic Horizon Manor is confirmed.`;
  ...
  async sendGiftCardEmail(recipientEmail, details) {
    ...
    const defaultTemplate = `Dear {{recipient_name}}, {{purchaser_name}} has sent you a luxury gift.`;
    const bodyContent = this._parseTemplate(dbTemplate || defaultTemplate, {
  ...
  async sendCheckInEmail(recipientEmail, booking) {
    ...
    const defaultTemplate = `Dear {{guest_name}}, Welcome to the Manor.`;
